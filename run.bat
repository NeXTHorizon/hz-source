...
IF defined IS_JAVA_IN_PATH (
	start "NHZ NRS" java -cp nhz.jar;lib\*;conf nxt.Nxt
) ELSE (
	IF EXIST "%PROGRAMFILES%\Java\jre7" (
		start "NHZ NRS" "%PROGRAMFILES%\Java\jre7\bin\java.exe" -cp nhz.jar;lib\*;conf nxt.Nxt
	) ELSE (
		IF EXIST "%PROGRAMFILES(X86)%\Java\jre7" (
			start "NHZ NRS" "%PROGRAMFILES(X86)%\Java\jre7\bin\java.exe" -cp nhz.jar;lib\*;conf nxt.Nxt
		) ELSE (
