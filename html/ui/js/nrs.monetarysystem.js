...
var NRS = (function(NRS, $, undefined) {
	...
	foundersModal.on("show.bs.modal", function(e) {
		...
		NRS.sendRequest("getCurrencyFounders", {
			...
			if (response.founders && response.founders.length) {
				...
				for (i = 0; i < response.founders.length; i++) {
					...
					rows += "<tr>" +
						...
						"</td>" +
						"<td>" + NRS.convertToNHZ(amountPerUnitNQT) + "</td>" +
						"<td>" + NRS.convertToNHZ(amountPerUnitNQT.multiply(new BigInteger(NRS.convertToQNTf(resSupply, decimals)))) + "</td>" +
			...
			rows += "<tr>" +
				"<td><b>Totals</b></td>" +
				"<td>" + NRS.convertToNHZ(totalAmountReserved) + "</td>" +
				"<td>" + NRS.convertToNHZ(totalAmountReserved.multiply(new BigInteger(NRS.convertToQNTf(resSupply, decimals)))) + "</td>" +
	...
	$("#currency_order_modal").on("show.bs.modal", function(e) {
		...
		try {
			...
			var totalNHZ = NRS.formatAmount(NRS.calculateOrderTotalNQT(unitsQNT, rateNQT), false, true);
		} catch (err) {
