...
IF defined IS_JAVA_IN_PATH (
	start "HORIZON NRS" java -cp classes;lib\*;conf nxt.Nxt
) ELSE (
	IF EXIST "%PROGRAMFILES%\Java\jre7" (
		start "HORIZON NRS" "%PROGRAMFILES%\Java\jre7\bin\java.exe" -cp classes;lib\*;conf nxt.Nxt
	) ELSE (
		IF EXIST "%PROGRAMFILES(X86)%\Java\jre7" (
			start "HORIZON NRS" "%PROGRAMFILES(X86)%\Java\jre7\bin\java.exe" -cp classes;lib\*;conf nxt.Nxt
		) ELSE (
