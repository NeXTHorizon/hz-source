...
public final class Generator {
    ...
    static long getHitTime(Account account, Block block) {
        return getHitTime(account.getEffectiveBalanceNHZ(), getHit(account.getPublicKey(), block), block);
    }
    ...
    private static long getHitTime(long effectiveBalanceNHZ, BigInteger hit, Block block) {
        return block.getTimestamp()
                + hit.divide(BigInteger.valueOf(block.getBaseTarget())
                .multiply(BigInteger.valueOf(effectiveBalanceNHZ))).longValue();
    }
    ...
    private void forge(int timestamp) {
        ...
        long effectiveBalance = account.getEffectiveBalanceNHZ();
        ...
        if (lastBlock.getHeight() < Constants.TRANSACTIONS_VERSION_1_BLOCK) {
            return;
        }
        ...
        if (! lastBlock.equals(lastBlocks.get(accountId))) {
            ...
            deadline = Math.max(getHitTime(account.getEffectiveBalanceNHZ(), hit, lastBlock) - Convert.getEpochTime(), 0);
